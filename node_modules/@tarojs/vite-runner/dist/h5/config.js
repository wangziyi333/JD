...
function default_1(viteCompilerContext) {
    ...
    let hmr = true;
    if ((0, shared_1.isBoolean)(serverOption.hmr)) {
    ...
    let cors = true;
    if ((0, shared_1.isBoolean)(serverOption.cors) || (0, shared_1.isObject)(serverOption.cors)) {
        cors = serverOption.cors;
    }
    let watch = {};
    if ((0, shared_1.isObject)(serverOption.watch)) {
        watch = serverOption.watch;
    }
    let strictPort = false;
    if ((0, shared_1.isBoolean)(serverOption.strictPort)) {
        strictPort = serverOption.strictPort;
    }
    let middlewareMode = false;
    if (serverOption.middlewareMode === 'ssr' || serverOption.middlewareMode === 'html') {
        middlewareMode = serverOption.middlewareMode;
    }
    let origin = '';
    if ((0, shared_1.isString)(serverOption.origin)) {
        origin = serverOption.origin;
    }
    let fsStrict = true;
    if (serverOption.fs && (0, shared_1.isBoolean)(serverOption.fs.strict)) {
        fsStrict = serverOption.fs.strict;
    }
    let fsAllow = [(0, vite_1.searchForWorkspaceRoot)(process.cwd())];
    if (serverOption.fs && Array.isArray(serverOption.fs.allow)) {
        fsAllow = serverOption.fs.allow;
    }
    let fsDeny = ['.env', '.env.*', '*.{crt,pem}', '**/.git/**'];
    ...
    else if ((0, shared_1.isString)(serverOption.allowedHosts) && serverOption.allowedHosts) {
        allowedHosts = [serverOption.allowedHosts];
    }
    let sourcemapIgnoreList = (sourcePath) => sourcePath.includes('node_modules');
