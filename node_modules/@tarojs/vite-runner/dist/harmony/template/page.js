...
class Parser extends base_1.default {
    constructor(appPath, appConfig, buildConfig, loaderMeta, isPure) {
        super();
    ...
    init() {
        var _a;
        this.tabbarList = ((_a = this.appConfig.tabBar) === null || _a === void 0 ? void 0 : _a.list) || [];
    ...
    renderState({ scope = ['page', 'tabbar'], decorator = '', name = '', type = '', prefix = '', suffix = '', connector = ', ', foreach = (_item, _idx) => '', disabled = false, }, isTabPage) {
        if (disabled || scope.length < 1)
            return null;
    ...
    getInstantiatePage(page) {
        ...
        if (!this.buildConfig.isBuildNativeComp && !this.isPure && entryOption) {
            ...
            structCodeArray.unshift(`@Entry(${this.prettyPrintJson(page instanceof Array ? page_1.TARO_TABBAR_PAGE_PATH : entryOption)})`);
        }
        const generateState = [
            this.renderState({
        ...
        if (this.isTabbarPage) {
            generateState.push('@State isTabBarShow: boolean = true', '@State tabBar: Partial<Taro.TabBar> = this.appConfig.tabBar || {}', '@State tabBarColor: string = this.tabBar.color || "#7A7E83"', '@State tabBarSelectedColor: string = this.tabBar.selectedColor || "#3CC51F"', '@State tabBarBackgroundColor: string = this.tabBar.backgroundColor || "#FFFFFF"', '@State tabBarBorderStyle: "white" | "black" = this.tabBar.borderStyle || "black"', '@State tabBarPosition: "top" | "bottom" = this.tabBar.position || "bottom"', '@State tabBarWithImage: boolean = this.tabBarList.every(e => !!e.iconPath)', '@State tabBarAnimationDuration: number = 400', '@State tabBarCurrentIndex: number = 0', 'private tabBarController: TabsController = new TabsController()');
        }
        ...
        if ((0, shared_1.isFunction)(modifyRenderState)) {
            modifyRenderState.call(this, generateState, page);
        }
        // 生成 build 函数内容
        let buildStr = this.renderPage(this.isTabbarPage, (_a = this.appConfig.window) === null || _a === void 0 ? void 0 : _a.enablePullDownRefresh, this.enableRefresh);
        if ((0, shared_1.isFunction)(modifyPageBuild)) {
            buildStr = modifyPageBuild.call(this, buildStr, page);
        }
        const generateMethods = [{
                name: 'getPageState',
                body: `const state = router.getState()
state.path ||= '${this.isTabbarPage ? page_1.TARO_TABBAR_PAGE_PATH : page.name}'
...
for (let i = 0; i < taskQueen.length; i++) {
  ...
  await new Promise<void>((resolve) => setTimeout(resolve, 16))
        ...
        if ((0, shared_1.isFunction)(modifyPageMethods)) {
        ...
        structCodeArray.push(this.transArr2Str(generateState, 2), '', this.transArr2Str(this.renderMethods(generateMethods).split('\n'), 2));
    ...
    generatePageHandleAppear(page) {
        ...
        if ((0, shared_1.isFunction)(modifyPageParams)) {
            paramsString = modifyPageParams.call(this, paramsString, page);
        }
        // FIXME window.__ohos.getLastWindow
        return `${(this.buildConfig.isBuildNativeComp || !0) ? '' : `if (${((_a = this.appConfig.window) === null || _a === void 0 ? void 0 : _a.navigationStyle) === 'custom'
...
callFn(this.page?.onReady, this, params)`}`;
    ...
    renderPage(isTabPage, appEnableRefresh = false, enableRefresh = 0) {
...
.backgroundColor(${isTabPage ? 'this.pageBackgroundContentColor[index] || this.pageBackgroundColor[index]' : 'this.pageBackgroundContentColor || this.pageBackgroundColor'} || "${((_b = this.appConfig.window) === null || _b === void 0 ? void 0 : _b.backgroundColorContent) || ((_c = this.appConfig.window) === null || _c === void 0 ? void 0 : _c.backgroundColor) || '#FFFFFF'}")
.clip(false)
.height('100%')
.scrollBar(typeof config${isTabPage ? '[index]' : ''}.enableScrollBar === 'boolean' ? config${isTabPage ? '[index]' : ''}.enableScrollBar : ${!((_d = this.appConfig.window) === null || _d === void 0 ? void 0 : _d.enableScrollBar) ? 'false' : 'true'})
.onAreaChange((_: Area, area: Area) => {
  const node: TaroElement | null = ${isTabPage ? 'this.node[index]' : 'this.node'}
  if (node) {
...
.onScrollStop(() => {
  ...
  const offset: TaroObject = ${isTabPage ? 'this.scroller[index]' : 'this.scroller'}?.currentOffset()
  const distance: number = config${isTabPage ? '[index]' : ''}.onReachBottomDistance || ${((_e = this.appConfig.window) === null || _e === void 0 ? void 0 : _e.onReachBottomDistance) || 50}
...
.backgroundColor(${isTabPage ? 'this.pageBackgroundColor[index]' : 'this.pageBackgroundColor'} || "${((_f = this.appConfig.window) === null || _f === void 0 ? void 0 : _f.backgroundColor) || '#FFFFFF'}")
.height('100%')
.width('100%')
...
.padding({
  bottom: px2vp(sysInfo.screenHeight - (sysInfo.safeArea?.bottom || 0)),
    ...
    generatePageAboutToAppear(page) {
        ...
        let appearStr = `${isBlended ? 'initHarmonyElement()\n' : ''}${this.transArr2Str([].concat(this.buildConfig.isBuildNativeComp || this.isPure || !entryOption ? [] : [
        ...
        if ((0, shared_1.isFunction)(modifyPageAppear)) {
            appearStr = modifyPageAppear.call(this, appearStr, page);
        }
        return appearStr;
    }
    generatePageAboutToDisAppear(page) {
...
this.removeTabBarEvent()` : 'callFn(this.page?.onUnload, this)'
        ...
        if ((0, shared_1.isFunction)(modifyPageDisappear)) {
    ...
    parse(rawId, page, name = 'TaroPage', resolve) {
        var _a;
        const { modifyResolveId } = this.loaderMeta;
        const { outputRoot = 'dist', sourceRoot = 'src' } = this.buildConfig;
        const targetRoot = node_path_1.default.resolve(this.appPath, sourceRoot);
        const entryOption = page instanceof Array ? page[0].entryOption : page.entryOption;
        const isBlended = this.buildConfig.blended || this.buildConfig.isBuildNativeComp || this.isPure || !entryOption;
        this.isTabbarPage = page instanceof Array;
        const pageRefresh = page instanceof Array
            ? page.filter(e => !!e).map(e => { var _a; return this.isEnable((_a = this.appConfig.window) === null || _a === void 0 ? void 0 : _a.enablePullDownRefresh, e.config.enablePullDownRefresh); })
            : [this.isEnable((_a = this.appConfig.window) === null || _a === void 0 ? void 0 : _a.enablePullDownRefresh, page === null || page === void 0 ? void 0 : page.config.enablePullDownRefresh)];
        ...
        let renderPath = node_path_1.default.posix.normalize(node_path_1.default.relative(node_path_1.default.dirname(rawId), node_path_1.default.join(targetRoot, 'render')));
        renderPath = renderPath.split(node_path_1.default.sep).join('/');
        if (/^[^./]/.test(renderPath)) {
        ...
        const importList = [
            ...
            '',
            isBlended ? __classPrivateFieldGet(this, _Parser_setReconciler, "f") : '',
        ...
        if (this.isTabbarPage) {
            importList.push(...this.tabbarList.map((e, i) => `import page${i}, { config as config${i} } from './${e.pagePath}/index${entry_1.TARO_COMP_SUFFIX}'`));
        }
        else {
            importList.push(`import createComponent, { config } from "${node_path_1.default.resolve(targetRoot, page.originName) + '/index' + entry_1.TARO_COMP_SUFFIX}"`);
        }
        if (isBlended && __classPrivateFieldGet(this, _Parser_setReconcilerPost, "f")) {
            importList.push(__classPrivateFieldGet(this, _Parser_setReconcilerPost, "f"));
        }
        const modifyPageImport = page instanceof Array ? page[0].modifyPageImport : page.modifyPageImport;
        if ((0, shared_1.isFunction)(modifyPageImport)) {
    ...
    parseEntry(rawId, page) {
        ...
        const pageName = (entryOption === null || entryOption === void 0 ? void 0 : entryOption.routeName) || page.name;
        ...
        const rawCode = this.transArr2Str([
            ...
            (page === null || page === void 0 ? void 0 : page.config.enableShareTimeline) ? 'component.enableShareTimeline = true' : null,
            (page === null || page === void 0 ? void 0 : page.config.enableShareAppMessage) ? 'component.enableShareAppMessage = true' : null,
            `export default () => ${createPageOrComponent}`,
        ]);
        return (0, shared_1.isFunction)(modifyEntryFile) ? modifyEntryFile.call(this, 'page', rawId, rawCode, page) : rawCode;
    }
}
