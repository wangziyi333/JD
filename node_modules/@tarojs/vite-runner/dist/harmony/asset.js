...
function fileToUrl(id, config, _pluginContext, viteCompilerContext, skipPublicCheck = false) {
    ...
    const resourceName = node_path_1.default.relative(appRoot, file).replace(ext, '').replace(/^[\\/]+/, '').replace(/[^A-z0-9]+/g, '_');
    const resourcePath = node_path_1.default.join((0, utils_1.escapePath)(outputRoot), '..', 'resources/base/media', `${resourceName}${ext}`);
